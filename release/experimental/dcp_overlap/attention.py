...
Kernel variants change output addresses, not attention/merge arithmetic.
No new quantizer, approximation, key selection, or softmax. Recorded v5
component tests establish bitwise parity; full-model qualification is separate.
"""
import ast
...
def make_head_attention(original):
    ...
    Decode calls the same _online kernel with unchanged split-K/BH/BN values.
    Split-merge/prefill variants change only store addresses, retaining key
    order and probability expansion; a CPU AST test verifies this invariant.
    """
