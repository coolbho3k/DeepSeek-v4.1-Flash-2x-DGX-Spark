...
"""Capture one uncached prefill chunk (shapes, no stacks) on an isolated pair.

Derived from ../model_fusion/profile.py; only the accepted profiler settings
differ. Stack collection adds CPU overhead: never use these traces for timing.
...
def long_prompt(index):
    import random
    rng = random.Random(90210 + index)
    words = ('river mountain signal ledger copper orbit meadow lantern quartz harbor '
             'cipher timber falcon marble glacier velvet ember prism anchor thistle').split()
    lines = [f'Record {i}: ' + ' '.join(rng.choice(words) for _ in range(14)) + f' code {rng.randrange(10**6)}.'
             for i in range(1100)]
    return 'Read the records, then reply OK.\n' + '\n'.join(lines)
...
def main():
    ...
    expected = dict(profiler='torch', max_iterations=1, delay_iterations=8,
    ...
    def request(index):
        payload = dict(model=speed.MODEL,
                       messages=[dict(role='user', content=long_prompt(index))],
                       max_tokens=2, temperature=0, seed=41,
        ...
        if not result.get('choices') or result['usage']['prompt_tokens'] < 18000:
            raise ValueError('Profile request did not reach the intended prefill depth')
