...
"""Clone a verified runtime and add prompt-lookup drafting on top of DSpark.

Adds serving/ds41/ngram_draft.py, applies exact edits from edits.json,
registers the module in the startup PINS and backend attestation, refreshes
source pins and writes a new immutable manifest plus receipt.
"""
import argparse
import ast
import hashlib
...
def main():
    ...
    shutil.copy2(HERE / 'ngram_draft.py', kit / 'serving/ds41/ngram_draft.py')
    for name, pairs in json.loads((HERE / 'edits.json').read_bytes()).items():
        for before, after in pairs:
            replace(kit / name, before, after)
    module_sha = sha((kit / 'serving/ds41/ngram_draft.py').read_bytes())
    for name, anchor, entries in (
            ('spark_combined_miaai.py', 'PINS = {', {'ds41.ngram_draft': module_sha}),
            ('spark_backend_attestation.py', 'PRIVATE_SOURCES = {', {'ds41/ngram_draft.py': module_sha})):
    ...
    requirements['ngram_draft'] = dict(ngram=3, window=65536, point_mass_draft_distribution=True,
