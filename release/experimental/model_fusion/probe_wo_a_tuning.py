    ...
    try:
        with torch.inference_mode():
            for layer in (0,19,39):
                ...
                for rows in (2,3,4):
                    for strided in (False,True):
                        ...
                        x=storage[:,:,:4096]
                        functions=dict(baseline=lambda:baseline(x,w,s))
                        functions.update({f'rows_n{n}_w{warps}': (lambda n=n,warps=warps:candidate.forward(x,w,s,tile_n=n,warps=warps)) for n in (8,16,32,64) for warps in (4,8)})
                        ...
                        for amplitude in (.0625,1.,8.):
                            ...
                            for name,(graph,value) in graphs.items():
                                ...
                                if not torch.equal(value,expected):
                                    ...
                                    failures.append(dict(layer=layer,rows=rows,strided=strided,amplitude=amplitude,
                                        variant=name,max_absolute=float(difference.abs().max()),different=int((value!=expected).sum())))
                                    if name=='baseline':raise ValueError('Changed baseline output')
                        timing={}
                        for cold in (False,True):
                            samples={name:[] for name in graphs}
                            for iteration in range(31):
                                names=list(graphs);shift=iteration%len(names);order=names[shift:]+names[:shift]
                                for name in order:
                        ...
                        row=dict(layer=layer,rows=rows,strided=strided,variant_failures=failures,
                                 exact_variants=[n for n in graphs if n not in {f['variant'] for f in failures}],
            ...
            report.update(status='component_measured_not_serving_qualified',
                          exact_variants=sorted(set.intersection(*(set(c['exact_variants']) for c in report['cases']))),peak_cuda_allocated_bytes=torch.cuda.max_memory_allocated())
