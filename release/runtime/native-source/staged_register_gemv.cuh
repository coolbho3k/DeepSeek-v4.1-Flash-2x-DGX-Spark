...
// Adapted from the retained MIT EXL3 exl3_gemv_kernel.cuh register loop.
#pragma once
#include "quant/exl3_gemv_kernel.cuh"

__device__ __forceinline__ void ds41_mma_ab_f(
    FragB a01,FragB a23,const FragB& b,FragC& c)
{
    FragA a;
    a[0]=a01[0];a[1]=a01[1];a[2]=a23[0];a[3]=a23[1];
    ptx_mma_m16n8k16(a,b,c); // FP16 operands, FP32 accumulation every MMA.
}
...
__device__ __forceinline__ void ds41_staged_gemv_inner(
    ...
    const int expert_block,const int expert_blocks)
{
    constexpr int CFG=0,MMODE=0,bits=3,cb=2;
    constexpr bool SMEM_STAGE=false,c_fp32=false;
    constexpr int size_m=1;
