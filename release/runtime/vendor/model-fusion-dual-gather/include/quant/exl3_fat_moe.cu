        ...
        for (int mb = 0; mb < MB; ++mb)
        {
            ...
            for (int rr = 0; rr < 2; ++rr)
            {
                ...
                if (r < rows_mb)
                {
                    ...
                    fm_ds41_round_half4(g);
                    fm_had_row(g, lane);
                    fm_ds41_round_half4(g);
                    fm_mul_half4(g, svh_g + lane * 4);
                    fm_ds41_round_half4(g);
                    fm_ds41_round_half4(u);
                    fm_had_row(u, lane);
                    fm_ds41_round_half4(u);
                    ...
                    // Match PackedExpert and the qualified thin-expert kernel:
    ...
    int64_t cb)
{
    ...
    launch_gateup<3, 2>(h13g, h13u, gate_ptrs, up_ptrs, gate_svh_ptrs, up_svh_ptrs, down_suh_ptrs, h2, row_weight, seg_expert, seg_row0, seg_rows, num_segs, lim, size_k, size_n, stream);
    cuda_check(cudaPeekAtLastError());
}
    ...
    int64_t cb)
{
    ...
    TORCH_CHECK(row_weight.is_cuda() && row_weight.scalar_type() == at::kFloat && row_weight.is_contiguous()
                && row_weight.size(0) >= h2.size(0), "row_weight must be float[rows_cap]");
