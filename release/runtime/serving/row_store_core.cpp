...
// Local derivative: page-contained packed rows and I/O accounting.
// Original source remains unmodified in release/runtime/vendor/miaai-dsv41-agpl.
// All local changes: AGPL-3.0-only; full credit to MiaAI for the reader/cache/pool.
...
constexpr uint64_t kPagePackedMagic = 0x3247504531345344ULL; // DS41EPG2
constexpr uint64_t kRowsPerPage = 15;
...
struct Store {
  ...
  bool packed_pages = false;
  std::atomic<uint64_t> io_bytes{0}, lookup_ns{0}, lookups{0};
...
static void read_from(Store *s, int fd, uint64_t offset, uint8_t *out, size_t length) {
  ...
  s->io_bytes.fetch_add(requested, std::memory_order_relaxed);
}
...
static void fetch_row(Store *s, uint64_t id, uint8_t *row) {
  ...
  if (s->packed_fd >= 0) {
    ...
    const uint64_t relative = id - s->row_lo;
    const uint64_t position = s->packed_pages
        ? (relative / kRowsPerPage) * kPage + (relative % kRowsPerPage) * kRowBytes
        : relative * kRowBytes;
    read_from(s, s->packed_fd, kPackedHeader + position, row, kRowBytes);
  } else {
...
extern "C" int row_store_attach_packed(Store *s, const char *path, uint64_t layer_id) {
  ...
  const bool page_packed = magic == kPagePackedMagic;
  uint64_t rows_per_page = 0, page_bytes = 0;
  std::memcpy(&rows_per_page, header + 48, 8);
  std::memcpy(&page_bytes, header + 56, 8);
  if (page_packed && (rows_per_page != kRowsPerPage || page_bytes != kPage))
    fail("invalid page-contained packed geometry");
  if ((magic != kPackedMagic && !page_packed) || layer != layer_id || lo != s->row_lo ||
  ...
  struct stat statbuf;
  if (fstat(fd, &statbuf) ||
      uint64_t(statbuf.st_size) < kPackedHeader + (page_packed
          ? ((hi - lo + kRowsPerPage - 1) / kRowsPerPage) * kPage
          : (hi - lo) * kRowBytes))
    fail("packed Engram shard is truncated");
  if (s->packed_fd >= 0) fail("packed shard already attached");
  s->packed_pages = page_packed;
  s->packed_fd = fd;
  return 1;
}
