// DS41 adaptation: group-32 FP8 NoPE, unchanged BF16 Q/RoPE math.
/*
 ...
 *   NOPE_DIM  = 448
 *   QUANT_BLOCK = 32 (UE8M0 FP8 quant block)
...
constexpr int kNumQuantBlocks = kNopeDim / kQuantBlock;   // 14
...
#pragma unroll
      ...
      float const absmax = fmaxf(warp2MaxAbs(local_absmax), 1e-4f);
...
#pragma unroll
        ...
        *reinterpret_cast<uint4*>(token_fp8_ptr + dim_base) =
        ...
        if ((laneId & 1) == 0) {
          int const q_block_idx = laneId >> 1;
...
static void launchFusedDeepseekV4Templated(
  ...
  STD_TORCH_CHECK(
      sm_version >= 80,
      "ds41_swa32_insert requires sm_80+ "
...
torch::stable::Tensor ds41_swa32_insert(
  ...
  STD_TORCH_CHECK(q_in.scalar_type() == torch::headeronly::ScalarType::BFloat16,
