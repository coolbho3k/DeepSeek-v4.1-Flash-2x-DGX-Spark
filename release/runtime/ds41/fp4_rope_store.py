...
"""Fused native V4.1 GPT-J RoPE and selectable NVFP4 main-cache store.
...
def _rope_insert(latent, positions, cos_sin, cache, slots,
    ...
    normed = tl.load(latent + token.to(tl.int64) * 512 + group_start * 16
                     + tl.arange(0, GROUPS * 16)).to(tl.float32)
    even, odd = tl.split(tl.reshape(normed, (GROUPS * 8, 2)))
    pair = group_start * 8 + tl.arange(0, GROUPS * 8) - 224
...
"""
    ...
    if count:
        groups, warps = _writer_geometry(count, compress_ratio)
        _rope_insert[(count, 32 // groups)](latent, positions, cos_sin_cache, kv_cache, slots,
